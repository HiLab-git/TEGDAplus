...
class Prototype_Pool(nn.Module):
    def __init__(self, class_num=4, max=50, scale_num=6):
...
class TTA(nn.Module):
    """TEGDA+ with attention mechanism for test-time adaptation"""
    ...
    def forward_and_adapt(self, x, model, optimizer, multi_eval=False):
        ...
        self.model_ema.encoder = selective_restore_fisher(self.model_ema.encoder, self.source_model.encoder, fisher_enc, revert_ratio=0.05*adapt_alpha)
        # self.model_ema.main_decoder_1 = selective_restore_fisher(self.model_ema.main_decoder_1, self.source_model.main_decoder_1, fisher_dec, revert_ratio=0.05*adapt_alpha)
