...
class Prompt(nn.Module):
    def __init__(self, prompt_alpha=0.03, image_size=128):
        super().__init__()
        ...
        self.init_para = torch.ones((1, 4, self.prompt_size, self.prompt_size, self.prompt_size))  # 3D prompt
    ...
    def forward(self, x):
        ...
        prompt = F.pad(self.data_prompt,
                       [self.padding_size, imgD - self.padding_size - self.prompt_size,
                        self.padding_size, imgH - self.padding_size - self.prompt_size,
...
class VPTTA(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(self.memory_bank.memory.keys()) >= self.neighbor:
            ...
            init_data = torch.ones((1, 1, self.prompt.prompt_size, self.prompt.prompt_size, self.prompt.prompt_size)).data
