...
class AETTA(nn.Module):
    ...
    def evaluate_dropout_2(self, input, curr_pred, net, n_iter=10, dropout=0.5):
        ...
        return aetta_WT_dice.item(),aetta_TC_dice.item(),aetta_ET_dice.item(), mismatch_mask, predictions, mean, est_avg_noweight
    ...
    def aetta(self, input, pred, model, multi_eval):
        est_WT,est_TC,est_ET, mismatch_mask, predictions, pred_mean, est_avg_noweight= self.evaluate_dropout_2(input, pred, model, dropout=0.4)
        est_WT = round(est_WT*100,2)
        est_TC = round(est_TC*100,2)
        est_ET = round(est_ET*100,2)
        est_avg = round((est_WT+est_ET+est_TC)/3,2)
        
        entropy = -(pred_mean.cpu().numpy()*np.log(pred_mean.cpu().numpy()+1e-10))
