...
class UNet(nn.Module):
    ...
    def get_output_attn(self, x, bank_features, case_weight):
        """Apply linear attention to the deepest encoder feature (2D)"""
        case_weight = torch.tensor(case_weight[:, None, None]).to('cuda').float()
        
        # Only process the deepest feature (last layer)
        if len(bank_features[-1]) == 0:
            # No bank features available, use original feature
            output, _ = self.dec1(x, self.blocks1)
            return output
        
        current_feat = x  # Already the last feature for 2D
        b, c, h, w = current_feat.shape
        
        # Reshape for attention: (B, C, H*W) -> (B, H*W, C)
        query = current_feat.flatten(2).permute(0, 2, 1)
        
        bank_stack = bank_features[-1] if isinstance(bank_features, list) else bank_features
        if isinstance(bank_stack, list) and len(bank_stack) == 0:
            output, _ = self.dec1(x, self.blocks1)
            return output
        ...
        # Blend with original feature based on confidence
        updated_feat = case_weight * current_feat + (1 - case_weight) * attn_output
        
        # Decode with updated feature
        output, _ = self.dec1(updated_feat, self.blocks1)
        return output
